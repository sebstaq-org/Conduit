...
- `packages/app-protocol` owns the shared UI<>backend protocol boundary.
- `packages/session-client` owns platform-neutral runtime client adaptation above that protocol.
- `packages/app-client` owns proof-surface client adaptation above that protocol.
- `packages/app-core` owns framework-neutral app-facing logic only.
...
- Feature and screen modules must not import `@conduit/app-protocol` or `@conduit/session-client` directly.
