...
/// Generates TypeScript/Zod contracts from backend-owned ACP serde types.
pub(crate) fn generate_typescript() -> Result<String, ContractError> {
    TypeScriptEmitter::new(ProtocolSchema::from_acp_types()?).emit()
}
...
impl ProtocolSchema {
    pub(crate) fn from_acp_types() -> Result<Self, ContractError> {
...
fn type_identifier(name: &str) -> String {
    let mut identifier = String::from("Acp");
