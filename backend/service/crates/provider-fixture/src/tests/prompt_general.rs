...
#[test]
fn session_prompt_selects_matching_prompt_for_same_session() -> TestResult<()> {
    let root = fixture_root(json!({ "sessions": [] }))?;
    write_session_prompt_capture(
        root.path(),
        SessionPromptCapture {
            capture: "first",
            prompt: vec![json!({ "type": "text", "text": "first" })],
            required_config: None,
            response: json!({ "stopReason": "end_turn" }),
            session_id: "session-1",
            updates: vec![transcript_update(0, "agent_message_chunk", "first-ready")],
        },
    )?;
    write_session_prompt_capture(
        root.path(),
        SessionPromptCapture {
            capture: "second",
            prompt: vec![json!({ "type": "text", "text": "second" })],
            required_config: None,
            response: json!({ "stopReason": "end_turn" }),
            session_id: "session-1",
            updates: vec![transcript_update(0, "agent_message_chunk", "second-ready")],
        },
    )?;
    let mut factory = FixtureProviderFactory::load(root.path())?;
    let mut port = factory.connect(ProviderId::Codex)?;
    ...
    if !updates
        .iter()
        .any(|update| super::support::value_contains_string(&update.update, "second-ready"))
    {
        return Err(format!("expected second prompt update, got {updates:?}").into());
    }
    Ok(())
}

#[test]
fn session_prompt_prefers_matching_config_fixture_for_same_prompt() -> TestResult<()> {
    let root = fixture_root(json!({ "sessions": [] }))?;
    write_session_set_config_option_capture(
        root.path(),
    ...
    write_session_prompt_capture(
        ...
        SessionPromptCapture {
            ...
            response: json!({ "stopReason": "end_turn" }),
            session_id: "session-1",
            updates: vec![transcript_update(0, "agent_message_chunk", "default-ready")],
        },
    )?;
    write_session_prompt_capture(
        root.path(),
    ...
    {
        return Err(format!("expected plan prompt update, got {updates:?}").into());
    }
    Ok(())
}
