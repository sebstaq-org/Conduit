...
The frontend workspace uses the locked desktop/mobile foundation shape: `apps/*` are shells, `packages/app-client` owns the app-facing client contract, `packages/app-core` owns framework-neutral vocabulary, and `packages/design-system-*` reserve UI boundaries without speculative product UI.
...
- `packages/app-client` owns frontend capability clients and proof-surface contracts.
